...
echo "$($PSQL "SELECT AVG(winner_goals + opponent_goals) AS average_total_goals_per_game FROM games")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON teams.team_id = games.winner_id WHERE team_id = winner_id ORDER BY name ASC")"
